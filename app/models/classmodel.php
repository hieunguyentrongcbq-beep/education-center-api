...
class classmodel {
    ...
    public function createEnrollment($data) {
        ...
        $conflicts = $scheduleService->checkStudentScheduleConflict((int)$studentId, (int)$classId);
        if ($conflicts) {
            return ['error' => ScheduleService::conflictClassMessage($conflicts, 'học'), 'details' => $conflicts];
        }
    ...
    public function updateEnrollment($id, $data) {
        ...
        $classId      = (int)($data['class_id'] ?? $enroll['class_id']);

        if ($newPayStatus === 'PAID' && ($enroll['payment_status'] ?? '') !== 'PAID') {
            $scheduleService = new ScheduleService();
            $conflicts = $scheduleService->checkStudentScheduleConflict((int)$studentId, $classId);
            if ($conflicts) {
                return ['error' => ScheduleService::conflictClassMessage($conflicts, 'học'), 'details' => $conflicts];
            }
        }
