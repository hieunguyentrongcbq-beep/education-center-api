...
class ScheduleService {
    ...
    public function checkStudentScheduleConflict(int $studentId, int $newClassId): array {
        ...
        $newSlots = $this->buildClassRegularSlots($newClass);
        if (!$newSlots) {
            return [];
        }

        $stmt = $this->db->prepare("
            SELECT e.class_id, e.payment_status
            FROM enrollments e
            WHERE e.student_id = :sid
              AND e.class_id != :newcid
              AND e.status = 'ACTIVE'
        ");
        $stmt->execute(['sid' => $studentId, 'newcid' => $newClassId]);

        $conflicts = [];
        $seen = [];
        foreach ($stmt->fetchAll() as $row) {
            $otherClassId = (int)$row['class_id'];
            $otherClass = $this->getClassWithCourse($otherClassId);
            if (!$otherClass) {
                continue;
            }

            $existingSlots = $this->getStudentClassRegularSlots($studentId, $otherClassId, $otherClass);
            foreach ($existingSlots as $ex) {
                foreach ($newSlots as $nw) {
                    if (!$this->slotsConflict($ex, $nw)) {
                        continue;
                    }
                    $key = ($ex['class_code'] ?? '') . '|' . ($ex['day_of_week'] ?? '');
                    if (isset($seen[$key])) {
                        continue;
                    }
                    $seen[$key] = true;
    ...
    /**
     * Lấy các buổi REGULAR của HV trong lớp: ưu tiên lịch đã tạo, không có thì lấy theo khóa học.
     */
    public function getStudentClassRegularSlots(int $studentId, int $classId, ?array $classRow = null): array {
        $class = $classRow ?: $this->getClassWithCourse($classId);
        if (!$class) {
            return [];
        }
        ...
        $stmt = $this->db->prepare("
            ...
            FROM schedules
            WHERE student_id = :sid AND class_id = :cid AND schedule_type = 'REGULAR'
            ORDER BY day_of_week
        ");
        $stmt->execute(['sid' => $studentId, 'cid' => $classId]);
        $rows = $stmt->fetchAll();
        if ($rows) {
            return array_map(function ($r) use ($class) {
                return [
        ...
        return $this->buildClassRegularSlots($class);
    }

    /** Tạo danh sách buổi REGULAR mặc định từ khóa học (thứ chính + thứ phụ). */
    private function buildClassRegularSlots(array $class): array {
        $start = $class['default_start_time'] ?? '18:00:00';
        $end = $class['default_end_time'] ?? '20:00:00';
        $slots = [];
        foreach ([(int)$class['day_primary'], (int)$class['day_secondary']] as $dow) {
            if ($dow <= 0) {
                continue;
            }
            $slots[] = [
    ...
    private function slotsConflict(array $a, array $b): bool {
        if (!$this->daysOverlap((int)$a['day_of_week'], (int)$b['day_of_week'])) {
            return false;
        }
        return $a['start_time'] < $b['end_time'] && $a['end_time'] > $b['start_time'];
    }

    private function daysOverlap(int $dayA, int $dayB): bool {
        return count(array_intersect(self::dayOfWeekVariants($dayA), self::dayOfWeekVariants($dayB))) > 0;
    }
